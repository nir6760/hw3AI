...
class ImprovedKNNForest:
    ...
    def fit(self, E_training, F_training, evaluatae_func=None, p_param=0.5, N_param=24, num_feauters_param=11, ):
    ...
    def classify_by_k(self, sample, k_param=15):
        ...
        dec_by = self.improved_forest.K/3
        votes_lst = [[classify_func(i) for num in range(int(dec_by - i / dec_by))] for i in
...
if __name__ == '__main__':
    ...
    knn_forest2.fit(E_train, F, evaluate_predict_err)
    im = knn_forest2.predict(E_test)
    print(im)
'''
    regular = []
    improved = []
    ...
    for i in range(5):
        ...
        knn_forest1.fit(E_train, F)
        r = knn_forest1.predict(E_test)
        regular.append(r)
